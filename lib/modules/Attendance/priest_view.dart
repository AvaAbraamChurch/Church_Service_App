...
///   Step 1 — User type        (أمناء الخدمة · خدام · مخدومين)
///   Step 2 — Attendance list  (4-status cards)
///
/// The attendance type (pageIndex) is passed from [PriestHomeView].
class PriestView extends StatefulWidget {
  final AttendanceCubit cubit;
  final int pageIndex;

  const PriestView(this.cubit, {super.key, this.pageIndex = -1});
...
class _PriestViewState extends State<PriestView> {
  ...
  void _showAddUserSheet() {
    ...
    // Filter to users of the same type that aren't already in the list
    final candidates = allUsers.where((u) {
      if (existing.contains(u.id)) return false;
      if (selectedUserType == superServant)
        return u.userType.code == UserType.superServant.code;
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (_, i) {
                        ...
                        return _AttendanceCard(
                          ...
                          onAdjustCoupons: (delta) async {
                            await widget.cubit.adjustCouponPoints(
                              userId: user.id,
                              delta: delta,
                              reason: 'تعديل نقاط الحضور',
                            );
                            // Sync local list with cubit's updated cache
                            if (mounted) {
                              final updated = widget.cubit.users?.firstWhere(
                                (u) => u.id == user.id,
                                orElse: () => user,
                              );
                              if (updated != null) {
                                setState(() {
                                  final sIdx = selectedGroupUsers.indexWhere(
                                    (u) => u.id == user.id,
                                  );
                                  if (sIdx != -1)
                                    selectedGroupUsers[sIdx] = updated;
                                  final fIdx = filteredUsers.indexWhere(
                                    (u) => u.id == user.id,
                                  );
                                  if (fIdx != -1) filteredUsers[fIdx] = updated;
                                });
                              }
                            }
                          },
...
class _AttendanceCard extends StatelessWidget {
  ...
  final ValueChanged<int> onAdjustCoupons; // +1 or -1
  const _AttendanceCard({
    required this.user,
    required this.status,
    required this.accentColor,
    required this.onStatusChanged,
    required this.onAdjustCoupons,
  });
