...
import '../../../core/blocs/attendance/attendance_states.dart';
...
class _VisitServantViewState extends State<VisitServantView> {
  ...
  void _fetchUsersFromFirestore() {
    ...
    // Build requested user types depending on visit type. For visiting (home)
    // include priests so they can be assigned as servants. However priests
    // typically don't have a userClass, so we need to fetch them separately
    // and merge the results.
    final types = <String>['CH', 'SV', 'SS'];
    final includePriests = selectedVisitType == VisitType.home;

    // Subscribe to non-priest users (filtered by class + gender)
    _usersSubscription = _usersRepository
        ...
        .listen(
          (users) {
            if (mounted && _usersSubscription != null) {
              setState(() {
    ...
    // If needed, fetch priests separately (they may not have userClass)
    if (includePriests) {
      _priestsSubscription = _usersRepository
          .getUsersByType('PR')
          .listen(
            (priests) {
              if (mounted && _priestsSubscription != null) {
                setState(() {
                  _fetchedPriests = priests;
                  // Keep priests as-is (do not filter by gender) so they appear for visiting
      ...
      // Ensure priests list is empty when not including them
      _fetchedPriests = [];
  ...
  List<UserModel> get servants {
    ...
    // If this is a visiting (home) visit, priests are also allowed to participate
    final allServants = _fetchedUsers.where((u) {
      // Priests should appear for visiting (home) visits regardless of gender
      if (u.userType == UserType.priest)
        return selectedVisitType == VisitType.home;

      // SuperServants and Servants must match the current user's gender
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (BuildContext context) => widget.attendanceCubit,
      child: BlocConsumer<AttendanceCubit, AttendanceState>(
        builder: (BuildContext context, state) {
          return ThemedScaffold(
            appBar: AppBar(title: const Text('الإفتقادات'), centerTitle: true),
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          // Selected Child Card with Contact Info
                          if (selectedChild != null) ...[
                            const SizedBox(height: 16),
                            _buildChildContactCard(selectedChild!),
                          ],
                                  ...
                                  onTap: () {
                                    if (mounted) {
                                      setState(() {
                                        ...
                                        // Re-fetch users since allowed servant types may change when visit type changes
                                        _isLoadingUsers = true;
                                      });
                                      // Fetch asynchronously after state updated
                            ...
                            Container(
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  final isSelected = selectedServants.contains(
                                    servant,
                                  );
                                  final isCurrentUser =
                                      servant.id == widget.currentUser!.id;
  ...
  Widget _buildChildContactCard(UserModel child) {
    final hasPhone = child.phoneNumber != null && child.phoneNumber!.isNotEmpty;
    final hasAddress = child.address != null && child.address!.isNotEmpty;

    return Container(
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [Colors.white, teal50],
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
        ),
        borderRadius: BorderRadius.circular(16),
          ...
          children: [
            ...
            if (hasPhone)
              _buildContactRow(
                ...
                onTap: () => _makePhoneCall(child.phoneNumber!),
              ),
          ...
          SnackBar(
            content: Row(
              children: const [
                Icon(Icons.error_outline, color: Colors.white),
                SizedBox(width: 12),
                Expanded(child: Text('لا يمكن فتح الخرائط')),
              ],
            ),
            backgroundColor: Colors.red,
            behavior: SnackBarBehavior.floating,
